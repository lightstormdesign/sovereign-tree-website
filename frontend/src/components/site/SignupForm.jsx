...
// ready.
export const SignupForm = ({ compact = false }) => {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <p className="font-body text-sm font-light text-[var(--st-text-soft)]">
          The guide is on its way — welcome to the family.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <p className="font-display text-2xl text-[var(--st-text)] sm:text-3xl">{SIGNUP_HEADLINE}</p>
      <p className="font-body mt-3 max-w-sm text-sm font-light leading-relaxed text-[var(--st-text-soft)]">
        {SIGNUP_SUBHEAD}
      </p>
      <p className="font-body mt-2 text-xs font-light text-[var(--st-text-muted)]">{SIGNUP_NOTE}</p>
